...
h = lgpio.gpiochip_open(0) #gpio setup
lgpio.gpio_claim_output(h, LASER_PIN, 0)
...
cam.configure(cam.create_preview_configuration(
    main={"format": "RGB888", "size": nativeRes}
))
cam.start()

# Use displayRes for detection
frameWidth, frameHeight = displayRes
...
previousTime = time.time() #gesture logging
gestureCooldown = 1.0
lastGestureTime = 0
killswitchCooldown = 1.5
lastKillTime = 0
validResetGestures = ["Aim", "Shoot"]
...
def logGesture(gesture, cx="", cy=""):
    timestamp = datetime.now().strftime("%H:%M:%S")
    csvWriter.writerow([timestamp, gesture, cx, cy])
...
def mapToServoAngle(cx, cy):
    ...
    panAngle = max(min(xAngle, 90), -90)  # [MODIFIED] allow both left/right yaw
    tiltAngle = max(min(yAngle + 90, 180), 0)
    return panAngle, tiltAngle

def setServoAngle(angle):
    angle = max(min(angle, 180), 0)
    pulseWidthUs = 500 + (angle / 180.0) * 2000
    dutyCycle = (pulseWidthUs / 20000) * 100
    lgpio.tx_pwm(h, SERVO_PIN, PWM_FREQ, dutyCycle)
...
def sendYawCommand(yawAngle):
    print(f"[MAVLINK] Sending yaw command: {yawAngle:.2f}° relative")

    direction = 1 if yawAngle >= 0 else -1  #explicit direction
    absYaw = abs(yawAngle)                 #magnitude only

    master.mav.command_long_send(
        master.target_system,
        master.target_component,
        mavutil.mavlink.MAV_CMD_CONDITION_YAW,
        0, absYaw, 30, direction, 1, 0, 0, 0)
...
while True:
# Step 1: Capture full-res image
    frameFull = cam.capture_array()
# Step 2: Resize to detection/display resolution
    frame = cv2.resize(frameFull, (frameWidth, frameHeight))
# Step 3: YOLO detection on resized frame
    results = model.predict(source=frame, conf=0.80, imgsz=256, agnostic_nms=True, verbose=False)
    boxes = results[0].boxes
# Step 4: Plot on same downscaled frame
    ...
    try:
        ...
        if gesture and gesture != previousGesture and (currentTime - lastGestureTime) > gestureCooldown:
            ...
            if gesture == "Killswitch":
                ...
                logGesture("Killswitch")

            elif killedStatus:
                if (currentTime - lastKillTime) > killswitchCooldown:
                    if gesture in validResetGestures:
                        print("SYSTEM UNLOCKED! Kill reset triggered via valid gesture.")
                        handleKillReset()
                        logGesture("AutoReset")
                        if gesture == "Aim":
                            handleAim(boxes)
                        elif gesture == "Shoot":
                            handleShoot()
                    else:
                        print(f"Ignored '{gesture}' — system is still locked.")
                else:
                    print("Ignored — killswitch cooldown not yet over.")
